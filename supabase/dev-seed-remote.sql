-- Dev seed for the REMOTE Supabase project (mvqfqifshnlfzneyftnz).
-- Run from https://supabase.com/dashboard/project/mvqfqifshnlfzneyftnz/sql
-- when you need realistic demo data without waiting for real activity.
--
-- This is the same dataset as `supabase/seed.sql` (which is local-only and
-- runs on `supabase db reset`), wrapped so it's idempotent: re-running drops
-- and recreates ONLY rows tagged with the sentinel UUID prefixes
-- (11111111-, 22222222-, 33333333-, 44444444-, 55555555-, 66666666-).
-- Any rows you've added manually through the UI are preserved.
--
-- Coverage: 2 owners, 2 buildings (Dakar), 8 units (mixed types/statuses),
-- 3 tenants, 4 active leases (residential + commercial), ~12 rent_dues
-- across the last 3 months (paid / partial / unpaid mix), matching payments,
-- 3 expenses.

----------------------------------------------------------------------
-- Clean prior seed (sentinel-scoped — never touches user-added rows)
----------------------------------------------------------------------
delete from payments where rent_due_id in (
  select id from rent_dues where id::text like '66666666-%'
);
delete from rent_dues where id::text like '66666666-%';
delete from expenses where
  building_id in (select id from buildings where id::text like '22222222-%')
  or unit_id in (select id from units where id::text like '33333333-%');
delete from leases where id::text like '55555555-%';
delete from units where id::text like '33333333-%';
delete from tenants where id::text like '44444444-%';
delete from buildings where id::text like '22222222-%';
delete from owners where id::text like '11111111-%';
...
-- Leases
----------------------------------------------------------------------
-- vat_amount and rent_incl_tax are DB-generated columns — never written.
...
union all select 'rent_dues', count(*) from rent_dues
